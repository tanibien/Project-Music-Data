...
// Оборачиваем импорт в функцию и вызываем её
async function setupConfig() {
  try {
    ...
    console.warn("Config not found (using placeholder):", e.message);
  }
}
...
async function init() {
  ...
  await setupConfig(); // Сначала пробуем подгрузить конфиг
  ...
  try {
    ...
    console.log("Initialization complete: users loaded.");
  } catch (error) {
...
function renderAllAnswers(events) {
  ...
  const topSongTime = getMostOften(
    eventsWithData,
    (e) => e.song_id,
    (e) => e.song.duration_seconds,
  );
  display(
    "Most listened to song (by listening time)",
    `${getSong(topSongTime[0]).artist} - ${getSong(topSongTime[0]).title}`,
  );

  // Q5: Longest streak
  let maxStreak = 0,
    currentStreak = 1,
    bestStreakSongId = events[0].song_id;
  ...
  if (currentStreak > maxStreak) {
    maxStreak = currentStreak;
    bestStreakSongId = events[events.length - 1].song_id;
  }
  const s = getSong(bestStreakSongId);
  display(
    "Longest streak song",
    `${s.artist} - ${s.title} (Streak: ${maxStreak})`,
  );

  // Q6: Every day songs
  const days = [...new Set(events.map((e) => e.timestamp.split("T")[0]))];
  const songsByDay = days.map(
    (day) =>
      new Set(
        events.filter((e) => e.timestamp.startsWith(day)).map((e) => e.song_id),
      ),
  );
  const everyDaySongIds = [...songsByDay[0]].filter((id) =>
    songsByDay.every((daySet) => daySet.has(id)),
  );
  if (everyDaySongIds.length > 0) {
    display(
      "Songs listened to every day",
      everyDaySongIds.map((id) => getSong(id).title).join(", "),
    );
  }

  // Q7: Top 3 genres
  const genreStats = {};
  eventsWithData.forEach(
    (e) => (genreStats[e.song.genre] = (genreStats[e.song.genre] || 0) + 1),
  );
...
function handleSearch(query) {
  ...
  filtered.forEach((song) => {
    display(
      `Artist Profile: ${song.artist}`,
      `Featured Track: ${song.title} | Genre: ${song.genre}`,
    );
  });
}
